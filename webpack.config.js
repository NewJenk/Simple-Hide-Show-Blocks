...
/**
 * Merge all files and blocks in a source folder.
 * 
 * Based upon link below.
 * 
 * @link https://github.com/webpack/webpack/issues/1732#issuecomment-163522781
 */
let frontend           = glob.sync('src/frontend/**/*.{js,css,scss}'),
    frontendJsChildren = glob.sync('src/frontend/**/*/index.js'),
...
/**
 * Take an array of file paths and add them to the config entry array ready to be built.
 * 
...
function toObject( pathsArray ) {
    ...
    pathsArray.forEach(function(paths) {
        ...
        let pathSplit      = paths.split('/'),                                // Split path to use in path.resolve
            fileName       = pathSplit[pathSplit.length - 1].split('.')[0],   // Remove ".file-extension" when building (e.g. style.css to style)
